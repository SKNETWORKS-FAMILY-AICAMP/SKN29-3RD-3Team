...
데이터 적재 흐름은 `Backend/src/preprocessing/build_all.py` 기준으로 구성되어 있다.

```mermaid
flowchart TD
    A["원천 데이터"] --> B["문서 유형별 Chunker"]
    B --> B1["law_chunker.py"]
    B --> B2["faq_chunker.py"]
    B --> B3["manual_chunker.py"]
    B --> B4["lh_guide_chunker.py"]
    B --> B5["web_faq_chunker.py"]
    B --> B6["md_table_chunker.py"]

    B1 --> C["law_chunks"]
    B2 --> D["faq_chunks"]
    B3 --> E["manual_chunks"]
    B4 --> F["lh_guide_chunks"]
    B5 --> G["web_faq_chunks"]
    B6 --> H["guide_chunks"]
...
전처리의 목적은 텍스트를 깨끗하게 만드는 것에 그치지 않는다. RAG 검색에서 의미 단위가 깨지지 않도록 문서 구조를 보존하는 것이 핵심이다.
...
flowchart TD
    A["원본 문서"] --> B["텍스트 추출"]
    B --> C{"추출 텍스트가 유효한가?"}
    C -->|"빈 페이지/디자인 페이지"| D["제외"]
    C -->|"텍스트는 있으나 반복 노이즈 포함"| E["헤더/푸터/목차 제거"]
    C -->|"정보가 이미지/표에만 있음"| F["보완 대상 표시"]
    E --> G{"문서 구조 기준"}
    G -->|"장/절/조/Q번호"| H["구조 기반 청킹"]
    G -->|"표 중심"| I["표 보존 또는 자연어 변환"]
    H --> J["metadata 부여"]
    I --> J
    F --> K["대체 추출 또는 수동 구조화 검토"]
```
...
청킹 전략은 모든 문서에 같은 크기 기준을 적용하지 않는다. 문서의 구조와 사용자의 질문 단위를 기준으로 다르게 설계한다.

| 문서 유형 | 기본 청킹 단위 | 분할 기준 | metadata 핵심 | 기대 효과 |
|---|---|---|---|---|
| 법령 | 조/항/호/목 | 조가 길면 항, 항이 길면 호/목으로 계층 분리 | `law`, `chapter`, `article`, `paragraph`, `item` | 법령 근거 추적 |
| 2024 FAQ | Q&A 쌍 | Q번호 기준, 질문과 답변을 함께 저장 | `chapter`, `section`, `q_number` | 사용자 질문에 직접 대응 |
| 업무 매뉴얼 | 본문은 절/소제목, Q&A는 질문쌍 | 1,500자 초과 절은 소제목 단위 분리 | `part`, `chapter`, `section`, `subsection` | 제도 설명과 FAQ 역할 분리 |
| LH 가이드 | 문서/섹션/공급유형/표 | 절차는 통합, 특별공급은 유형별, 표는 별도/자연어화 | `page`, `supply_type`, `section_title` | 최신 LH 기준 검색 |
| 웹 FAQ | Q&A 쌍 | JSON 항목 단위 | `scope`, `category`, `subcategory` | 실무 FAQ 보강 |
| 청약제도안내 MD | heading + 문단 | H1/H2/H3 기준, 표는 자연어 변환 후 분리 | `Header_1`, `Header_2`, `Header_3` | 표 기반 제도 정보 검색 |

### 5.1 법령 청킹

법령은 글자 수로 자르면 안 된다. 조문은 장, 조, 항, 호, 목의 계층이 의미를 만든다. 따라서 `law_chunks`는 계층 구조를 metadata에 남기고, 하위 단위 청크에는 상위 문맥을 prefix로 포함한다.
...
FAQ는 질문과 답변을 분리하지 않는다. 질문만 저장하면 답변 근거가 사라지고, 답변만 저장하면 어떤 질문에 대한 답인지 약해진다. 따라서 `Q번호 + 질문 + 답변`을 하나의 chunk로 저장한다.
...
LH 가이드는 최신 수치와 실무 안내가 강점이다. 특히 소득기준, 자산기준, 전매제한, 거주의무처럼 숫자 정보가 많아 표를 그대로 embedding하면 질문 문장과 형태가 맞지 않을 수 있다. 따라서 표는 의미 단위로 묶거나 자연어 문장으로 바꾸어 저장한다.
...
`web_faq_chunks`는 청약홈과 마이홈포털 FAQ JSON을 Q&A 쌍으로 변환한 collection이다. PDF처럼 페이지 구조를 해석하는 대신, JSON 항목의 질문/답변 필드를 그대로 의미 단위로 사용한다.
...
Markdown 안내 문서는 표 비중이 높다. 기존 표 구조는 `|`, `---`, 강조 문법이 많아 자연어 질문과 embedding 거리가 멀어지는 문제가 있었다. v2 전략은 표를 자연어 문장으로 변환한 뒤 heading 기반 청킹을 적용한다.
...
Retriever는 6개 collection을 단순히 동일 비중으로 검색하지 않는다. 질문 키워드에 따라 우선 collection을 정하고, 우선 collection에서는 더 많은 후보를 가져오며, 나머지 collection에서도 최소 후보를 가져와 검색 누락을 줄인다.

```mermaid
flowchart TD
    A["사용자 질문"] --> B["route_collections()"]
    B --> C{"키워드 매칭"}
    C -->|"전매제한/거주의무"| D["lh_guide_chunks 우선"]
    C -->|"가점/무주택기간"| E["guide_chunks 우선"]
    C -->|"소득기준"| F["lh_guide_chunks + guide_chunks 우선"]
    C -->|"특별공급"| G["lh_guide_chunks + web_faq_chunks + faq_chunks 우선"]
    C -->|"매칭 없음"| H["전체 collection 동일 검색"]
    D --> I["search_weighted()"]
    E --> I
    F --> I
    G --> I
