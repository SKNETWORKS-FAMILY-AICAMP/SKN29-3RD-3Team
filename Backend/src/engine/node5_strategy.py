...
2. create_react_agent: 나머지 툴 자유 호출 (현재 mock)
...
from tools.rag_tools import check_regional_priority, analyze_subscription_timing
...
# ── Mock 툴 (나중에 실제 구현으로 교체) ──────────────────────────

@tool
def generate_final_strategy(context: str) -> str:
    """최종 전략 생성 (Mock - 추후 RAG + 추론 연동 예정)"""
    return f"최종 전략 생성 결과 (추후 구현 예정): {context}"
...
5. generate_final_strategy 툴 호출
"""
