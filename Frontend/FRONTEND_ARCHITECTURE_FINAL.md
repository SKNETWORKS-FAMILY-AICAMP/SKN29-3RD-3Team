...
이 문서는 `origin/final`을 받아온 직후의 프론트엔드 구조를 공유하기 위한 설명서다. 현재 워크벤치(`final_front_debug`)에는 테스트 편의를 위한 디버그 프리셋과 발표용 UI 개선이 추가되어 있으며, 그 외 프론트 구조와 백엔드 연동 방식은 `final` 기준으로 설명한다.

현재 서비스 표시명은 `청약 랭그래프 시스템`이다.
